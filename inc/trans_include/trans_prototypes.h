...
void shared_memory_write(
    unsigned long long size, unsigned long long checksum, int block_num, int finished);
...
void transdirlist_add(char *file_name, unsigned long long size);

/* Add file name to transdirlist and include directory path */
void transdirlist2add(char *dir_name, char *file_name, unsigned long long size);

/* Log to the transdirlist file */
void transdirlist_log(char *file_name, char *file_ptr, long long size);
...
int edc_approve(char **argv, int argc);

/* Set server to one cloest to the customer */
int closest_server_set(void);
...
void trans_release_info_print(char **argv, int ret);
...
#endif /* define for trans_prototypes_h */
