...
void ftp_send(char *remote_dir, char *description, char send_email, char *email_string, char *log_string);
...
void read4company_big_file(char *key, char lines[TRANS_LINE_NUM][TRANS_LINE_LEN], TRANS_FILE_STAT *fs);
...
char * customer_msg_set(char *email_addr);
...
int put_parallel_block(char *local_file, char *remote_file, char *remote_dir, 	int total_blocks, int *total_size); 
