...
int main(int argc, char *argv[])
{
    ...
    if ((decrypts != 0 && user == NULL) || pwd == NULL)
    {
         help();
         return 0;
    }
    ...
    if (decrypts)
    {
         password_decrypt(crypted_password, pwd);
         printf ("\n\t%s\n", crypted_password);
    }
    else if (generates_icomrc)
    {
         if (create_icomrc(NULL, user, pwd))
         {
              printf("file .icomrc generated.\n" );
         }
         else
         {
              printf("ERROR on generating .icomrc file\n" );
         }
    }
    else
    {
         encrypt_ascii_pwd(crypted_password, pwd);
