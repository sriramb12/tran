/****************************************************************************
 *
 * Function:	transrc_read
 *
 * Description:	Routine to read the .transrc or .icomrc file.
 *
 *		the settings in this file allow the user to configure how the software runs.
 *
 ***************************************************************************/
...
/* If using generic version, .icomrc file must exist */
...
void transrc_read(char *rc_file)
{
    ...
    /* Print out amount transcended every 16 mb. */
    transrc.hashBySize = 16*1024*1024;
...
#ifndef GENERIC
    DBG("GENERIC not defined during build, return on next line");    
    return;
#else
    icomrc_build(file_name);

    if ((fp = fopen(file_name, "r")) == NULL)
    {
	    printf("Unable to get server info\n");
	    exit(1);
    }
#endif
